...
def conquer(polyA, polyB):
    ...
    # Recursive Case
        ...
        multiplication = shift(conquer(middle1, middle2), len(polyA) // 2)
...
def extend(poly, number):
    poly.extend([0] * number)
    return poly


# Function to handle addition: poly1 + poly2
def addition(poly1, poly2):
    return [a + b for a, b in zip(*adjust(poly1, poly2))]
...
#List comprehension
#zip with * to acces tuple return values and quicker for loop
#got rid of redundant variables
#more extend in place of for loops
#implicit for loops (sum)